...
- 作成/更新/削除するファイルのツリー
  - どこにファイルが作成されるか、どのファイルが更新/削除されるか視覚的に分かり易いように、ツリーも含めてください
- データフロー(必要に応じて)
  - ロジックがある程度複雑な場合はデータフローを記載してください。
    箇条書き、ASCIIアート、Mermaidなど、表現し易い方法で構いません。
- 必要になるテストの観点
...
作成したSOWの内容をもう一度確認し、"実装するにあたり不足している情報がないか"確認してください。
不足している情報はユーザーに尋ねて下さい。
