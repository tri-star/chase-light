...
- マイグレーションSQLファイルは手動で作成しない
- `schema.ts` の変更から Drizzle Kit が自動的にマイグレーションファイルを生成
- `pnpm --filter backend db:generate` でマイグレーションファイルを生成
- `pnpm --filter backend db:migrate` でマイグレーションを実行
- 生成されたマイグレーションファイルは必ず内容を確認してから実行する
...
- UNIQUE制約: 利用する。(トランザクション等も考慮するとDB側でなければ正確に担保出来ないため)
- 外部キー制約: 利用する。(DrizzleなどのORMを使用する場合、外部キー制約はORM側での整合性チェックを補完するために有用)
- CHECK制約: 利用しない。(アプリケーション側で管理する範疇と考える)
