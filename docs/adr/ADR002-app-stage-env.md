...
環境を識別するために、`APP_STAGE` という環境変数を使用するアプローチを検討します。この環境変数の値（例: `development`, `staging`, `production`）に基づいて、アプリケーションが自身の実行環境を判断し、適切な設定を自動的に読み込むようにします。
...
- **実装方針**:
  ...
  - 環境変数の値に応じて、対応する設定ファイル（例: `config/development.json`, `config/production.json`）をロードします。
  - `APP_STAGE` が未設定の場合は、デフォルトで `development` として扱います。
