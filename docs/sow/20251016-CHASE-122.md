...
- `features/notification`
  ...
  - **application/use-cases/**
    ...
    - `notification-schedule.service.ts`（ドメインサービス／純関数）
  - **infra/repositories/**
...
6. **`packages/backend/src/features/notification/application/services/notification-schedule.service.ts`**
...
<!-- REVIEW: UseCaseに対するテストは不要なので、削除してください。Presentation層でのテストと重複し易いことや、Repositoryをモックするとテストの意味が希薄になるため。 -->
12. **`packages/backend/src/features/notification/application/use-cases/__tests__/generate-digest-notifications.use-case.test.ts`**
13. **`packages/backend/src/features/notification/application/services/__tests__/notification-schedule.service.test.ts`**
14. **`packages/backend/infrastructure/events/notification/generate-digest-notifications.json`**
