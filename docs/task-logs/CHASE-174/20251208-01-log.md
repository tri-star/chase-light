...
- [x] Phase1-3: 関連箇所の型エクスポートやDI配線を確認（必要に応じて後続フェーズで対応）
...
- [x] Phase3-2: OpenAPI更新・サーバー全体への配線確認（次セッション）
...
- ワーカーエクスポート: `translate-activity-body/index.ts` を追加し、他ワーカーと同様に `handler` を公開。
- OpenAPI: ローカルサーバー（PORT=3005）起動のうえ `pnpm --filter backend openapi:update` を実行し、`packages/backend/openapi.json` を翻訳ルートを含む最新状態に更新。
- 検証: `pnpm lint` / `pnpm test` を再実行し、全パッケージで成功。
