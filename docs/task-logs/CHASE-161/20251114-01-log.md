...
2. **PR作成**
   - ブランチをリモートにプッシュ
   - PR #173を作成: https://github.com/tri-star/chase-light/pull/173
   - PRタイトル: "CHASE-161: ClFabButtonに配置責務とz-indexトークンを追加"
...
- デザイントークンシステムにzIndexサポートを追加（z-fab: 1050）
- ClFabButtonに固定配置機能を追加（position, offsetX, offsetY props）
- レスポンシブオフセット実装（モバイル: 5rem, sm+: 10rem）
- Storybookに配置バリエーションのストーリーを追加
- DataSourceFabButton.vueを削除し、DashboardPage.vueで直接ClFabButtonを使用

### コミット履歴

1. c4bab80 - feat(frontend): デザイントークンとスクリプトにzIndexサポートを追加
2. e922118 - feat(frontend): ClFabButtonに固定配置機能とz-indexトークンを適用
3. cf1d90b - feat(frontend): ClFabButtonのStorybookストーリーに配置バリエーションを追加
4. f107108 - refactor(frontend): CHASE-161 DashboardPageでClFabButtonを直接使用するよう変更
