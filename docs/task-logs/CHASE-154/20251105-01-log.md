...
- [x] Step 3: Phase 3 — ダッシュボード統合
  - [x] `packages/frontend/components/pages/dashboard/parts/AddDataSourceModal.vue` と関連テスト/Stories/VRTを実装
  - [x] `packages/frontend/components/pages/dashboard/parts/DataSourceFabButton.vue` と関連テスト/Storiesを実装
  - [x] `packages/frontend/components/pages/dashboard/DashboardPage.vue` を更新し、FAB/モーダル制御、`refreshNuxtData` を結線
  - [x] `packages/frontend/tests/dashboard.authenticated.spec.ts` を拡張してデータソース追加シナリオを追加（実行はユーザーに依頼）
  - [x] Storybook/VRT テストコードを更新して新しいUIをカバー
...
### 2025-11-05 Phase 3
- `packages/frontend/components/pages/dashboard/parts/AddDataSourceModal.vue` を新設し、GitHub URL バリデーション、POST `/api/data-sources` 呼び出し、成功/失敗トーストを内包。Story/VRT 用に `initialValues` もサポート。
- `AddDataSourceModal` の単体テストで `$fetch`・`useToast` をモックし、成功・失敗時のハンドリングや `showModal` ポリフィルを検証。
- `DataSourceFabButton` を追加し、固定配置の FAB UI とテスト/Story を整備。
- `DashboardPage` に FAB/モーダルの制御と `refreshNuxtData('dashboard-data-sources'|'dashboard-notifications')` を実装。ユニットテストで `useFetch` をモックしイベント連携を確認。
- Storybook にデータソース POST ハンドラとモーダル操作用のストーリーを追加し、VRT にモーダルオープン状態と `AddDataSourceModal` の撮影を追加。
- Playwright `dashboard.authenticated.spec.ts` にデータソース追加シナリオを追記し、POST をモックしてトースト表示まで検証（実行はユーザーに依頼）。
- コマンド実行: `pnpm --filter frontend format`, `pnpm --filter frontend lint`, `pnpm --filter frontend test`。
