...
- `routes.ts` のシグネチャ変更: 個別のUseCaseパラメータから `ActivityDepsOverrides` を受け取る形式に変更
- 内部で `buildActivityDeps` を呼び出して依存関係を構築
- `index.ts` の本番コード更新: buildActivityDepsを使用した簡潔な実装に変更
- 必要なエクスポートを整理

⚠️ テストコードはまだ古いシグネチャを使用しているため、Phase 3で修正が必要

### Phase 3: 完了内容

✅ 完了

- `create-activity-test-app.ts` 作成: テスト用のHonoアプリを1行で初期化できるヘルパー関数を実装
- 既存のテストファイルをリファクタリング:
  - `activities/__tests__/index.test.ts`: 初期化コードを20行以上から3行に削減
  - `activities/__tests__/translation.test.ts`: 同様に簡素化
  - `data-source-activities/__tests__/index.test.ts`: 同様に簡素化
- format, lint, test実行: 全て成功
...
- 全テスト実行: 25ファイル、189テスト全て成功
- format, lint実行: エラーなし
- ブランチ: feature/CHASE-185 で作業
- コミット:
  1. Phase 1: 依存関係の型定義と組み立て関数を追加 (677e3cb)
  2. Phase 2+3: ルート関数のシグネチャ変更とテストヘルパー追加 (91fde7b)
- 次: push と PR作成
