...
export class UserProfileService {
  ...
  async updateUserProfile(id: string, data: UpdateUserRequest): Promise<User | null> {
...
export class UserPreferenceService {
  ...
  async updatePreferences(userId: string, data: UpdatePreferenceRequest): Promise<UserPreference> {
...
export class UserNotificationService {
  ...
  async sendUserNotification(userId: string, notification: NotificationRequest): Promise<void> {
    // 通知送信の業務ルール
...
export const userPreferenceService = new UserPreferenceService(new UserPreferenceRepository())
export const userNotificationService = new UserNotificationService(new NotificationRepository())
```
...
export class UserRepository extends DrizzleBaseRepository<User> {
  ...
  async findMany(
    ...
    if (filters?.githubUsername) {
      whereConditions.push(eq(users.githubUsername, filters.githubUsername as string))
    }
...
export type CreateUserData = Omit<InferInsertModel<typeof users>, 'id' | 'createdAt' | 'updatedAt'>
...
```typescript
// features/users/__tests__/services.test.ts
...
describe('UserService', () => {
    ...
    test.each([
      {
        ...
        expected: { id: 'uuid-1', name: '田中太郎', email: 'tanaka@example.com' }
      },
      {
        input: { name: 'John Doe', email: 'john@example.com', githubUsername: 'johndoe' },
        expected: { id: 'uuid-2', name: 'John Doe', email: 'john@example.com', githubUsername: 'johndoe' }
      }
    ])('正常なユーザーデータでユーザーを作成: %o', async ({ input, expected }) => {
  ...
  describe('境界値テスト', () => {
    test.each([
      ...
      const result = await userService.getUserById(input as string)
...
export const validateSchema = (target: 'json' | 'query' | 'param', schema: ZodSchema) =>
...
- [ ] テストが__tests__フォルダに配置され、日本語で記述されているか
