...
import { MessageProcessor, createMessageHandler } from '../lib/elasticmq-poller/message-processor.js'
...
async function runTests() {
  ...
  test('Body なしメッセージのエラー処理', async () => {
    ...
    assertTrue(result.error.message.includes('Body'), 'Error should mention missing Body')
  })
  ...
  test('Lambda関数名解決エラー', () => {
    ...
    return processor.processMessage(mockMessage)
      .then((result) => {
        // エラーが発生して success: false になることを確認
        assertEquals(result.success, false)
        assertExists(result.error)
        assertTrue(result.error.message.includes('Lambda関数名が見つかりません'))
      })
  })
  ...
  await test('Lambda接続エラーの処理', async () => {
    ...
    try {
      ...
      if (!result.success) {
        assertExists(result.error)
        console.log(`   🔌 接続エラーが正しく処理されました: ${result.error.message}`)
      } else {
      ...
      // 接続エラーも正常な動作として扱う
      console.log(`   🔌 接続エラーが発生しました（SAM Localが起動していない可能性があります）`)
    }
  })
  ...
  test('大きなメッセージの処理', () => {
    const largeData = {
      ...
      data: Array(1000).fill('test-data-item').map((item, index) => ({
        id: `${item}-${index}`,
        value: `value-${index}`,
        timestamp: new Date().toISOString()
      }))
    ...
    console.log(`   📦 大きなメッセージ（${largeMessage.Body.length} バイト）の処理を確認`)
  })
