...
import { type PollerConfig, getLambdaFunctionName } from './config.js'
...
export class MessageProcessor {
  ...
  async processMessage(message: Message): Promise<MessageProcessorResult> {
    try {
      // キューに対応するLambda関数名を取得
      const functionName = getLambdaFunctionName(this.config.queueName)
      ...
      const functionName = this.getFunctionNameSafely()
   ...
   * Lambda関数を実行
   */
  private async invokeLambdaFunction(functionName: string, payload: unknown): Promise<unknown> {
    ...
    if (response.FunctionError) {
      const errorPayload = response.Payload ? 
        ...
        'Unknown error'
      throw new Error(`Lambda関数エラー: ${response.FunctionError} - ${JSON.stringify(errorPayload)}`)
    }
  ...
  private extractPayload(message: Message): unknown {
    ...
    try {
      ...
      logger.warn('メッセージBodyのJSONパースに失敗しました。文字列として処理します。', error)
      return { rawMessage: message.Body }
    }
  }

  /**
   * 安全にLambda関数名を取得（エラー時用）
   */
  private getFunctionNameSafely(): string {
    try {
      return getLambdaFunctionName(this.config.queueName)
    } catch {
      return `unknown-function-for-${this.config.queueName}`
    }
  }
}
