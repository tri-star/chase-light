...
// デフォルト設定
const DEFAULT_CONFIG: Omit<PollerConfig, 'queueName'> = {
...
export function loadLocalVariables(): LocalVariables {
  ...
  const localVariablesPath = path.join(backendDir, 'infrastructure/local-variables.json')
...
export function createPollerConfig(args: CommandLineArgs): PollerConfig {
  ...
  if (!queueConfig) {
    console.warn(`警告: キュー '${args.queue}' の設定が見つかりません。デフォルト設定を使用します。`)
  }
  ...
  const config: PollerConfig = {
    ...
    elasticMqEndpoint: localVariables.ElasticMQ?.ExternalEndpoint || DEFAULT_CONFIG.elasticMqEndpoint,
  }
...
export function getLambdaFunctionName(queueName: string): string {
  ...
  throw new Error(`キュー '${queueName}' に対応するLambda関数名が見つかりません`)
}
