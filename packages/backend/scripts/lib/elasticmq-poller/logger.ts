...
export const logger = {
   ...
   * 受信したメッセージを見やすい形式で出力
   */
  messageReceived: (queueName: string, messageBody: string, messageId?: string) => {
    console.log(formatMessage('info', `メッセージを受信しました (キュー: ${queueName})`))
    if (messageId) {
   ...
   * Lambda実行結果を出力
   */
  lambdaResult: (functionName: string, success: boolean, result?: unknown, error?: Error) => {
    if (success) {
      console.log(formatMessage('success', `Lambda関数実行成功: ${functionName}`))
      if (result) {
   ...
   * ポーラー開始ログ
   */
  pollerStarted: (queueName: string, config: { pollIntervalMs: number; elasticMqEndpoint: string }) => {
