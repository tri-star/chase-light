...
import { USER_NAME } from "../../../constants/identity.constants"
import { USER_EMAIL } from "../../../constants/identity.constants"
 ...
 * プロフィールルートファクトリー
 */
export const createProfileRoutes = (app: OpenAPIHono) => {
  // UseCaseの依存関係を構築
  ...
  app.openapi(getProfileRoute, async (c) => {
    try {
      ...
      // プロフィール取得
      const user = await getUserProfileUseCase.getUserProfileByAuth0Id({
        auth0UserId,
      })
      if (!user) {
  ...
  app.openapi(updateProfileRoute, async (c) => {
    try {
      ...
      // プロフィール更新
      const updatedUser = await updateUserProfileUseCase.updateUserProfile({
