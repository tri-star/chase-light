...
test.describe('Protected API Endpoints', () => {
  test('should access protected test endpoint with authentication', async ({ request }) => {
  ...
  test('should access GitHub user API (expect error with test token)', async ({ request }) => {
  ...
  test('should access GitHub repos API (expect error with test token)', async ({ request }) => {
    // テスト環境では実際のGitHubトークンがないため、エラーレスポンスを期待
