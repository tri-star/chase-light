...
test.describe('Authenticated Dashboard', () => {
  test('should display dashboard with user information', async ({ page }) => {
    ...
    // ユーザー情報が表示されることを確認
    await expect(page.locator('text=Welcome, Playwright Test User')).toBeVisible()
    await expect(page.locator('text=Logout')).toBeVisible()
  })

  test('should display user information in details section', async ({ page }) => {
    await page.goto('/dashboard')
    ...
    await expect(page.locator('dd').filter({ hasText: 'Playwright Test User' })).toBeVisible()
  ...
  test('should be able to test protected API', async ({ page }) => {
    ...
    const protectedApiButton = page.locator('[data-testid="protected-api-button"]');
    ...
    await expect(protectedApiButton).toHaveText('Test Protected API', { timeout: 10000 })

    // API結果のコンテナが表示されるかチェック
    const apiResultContainer = page.locator('div').filter({ hasText: /^(Error:|{)/ }).first()
    ...
    if (await apiResultContainer.isVisible()) {
      ...
      if (apiResponse?.includes('Error:')) {
        ...
        expect(apiResponse).toContain('Protected API endpoint accessed successfully')
