  ...
  <div>
      ...
      <div class="animate-pulse space-y-6">
        <div v-for="i in 3" :key="i" class="space-y-4">
          <div class="flex items-center justify-between">
            <div class="rounded h-4 w-24 bg-surface-secondary-default"></div>
            <div
              class="h-2 w-2 rounded-full bg-surface-secondary-default"
            ></div>
          </div>
          <div class="space-y-2">
            <div class="rounded h-4 w-3/4 bg-surface-secondary-default"></div>
            <div class="rounded h-3 w-1/2 bg-surface-secondary-default"></div>
            <div class="rounded h-3 w-5/6 bg-surface-secondary-default"></div>
          </div>
        </div>
      </div>
    </div>
    ...
    <div v-else-if="props.error" class="p-6">
      <div class="text-center">
        ...
        <h3 class="text-sm mt-2 font-medium text-card-value">
          通知の読み込みに失敗しました
        </h3>
        <p class="text-sm mt-1 text-card-value opacity-75">
          {{ props.error }}
        </p>
      </div>
    </div>
