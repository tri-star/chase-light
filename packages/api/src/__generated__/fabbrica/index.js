...
const modelFieldDefinitions = [{
        ...
        fields: [{
                ...
                relationName: "NotificationToNotificationItem"
            }]
    }, {
...
function autoGenerateNotificationScalarsOrEnums({ seq }) {
    return {
        ...
        read: getScalarFieldValueGenerator().Boolean({ modelName: "Notification", fieldName: "read", isId: false, isUnique: false, seq })
    };
}
...
function isNotificationItemNotificationFactory(x) {
    return x?._factoryFor === "Notification";
}
function autoGenerateNotificationItemScalarsOrEnums({ seq }) {
    return {
        id: getScalarFieldValueGenerator().String({ modelName: "NotificationItem", fieldName: "id", isId: true, isUnique: false, seq }),
        feedLogId: getScalarFieldValueGenerator().String({ modelName: "NotificationItem", fieldName: "feedLogId", isId: false, isUnique: false, seq }),
        title: getScalarFieldValueGenerator().String({ modelName: "NotificationItem", fieldName: "title", isId: false, isUnique: false, seq })
    };
}
function defineNotificationItemFactoryInternal({ defaultData: defaultDataResolver, onAfterBuild, onBeforeCreate, onAfterCreate, traits: traitsDefs = {} }, defaultTransientFieldValues) {
    const getFactoryWithTraits = (traitKeys = []) => {
        const seqKey = {};
        const getSeq = () => getSequenceCounter(seqKey);
        const screen = createScreener("NotificationItem", modelFieldDefinitions);
        const handleAfterBuild = createCallbackChain([
            onAfterBuild,
            ...traitKeys.map(traitKey => traitsDefs[traitKey]?.onAfterBuild),
        ]);
        ...
        const build = async (inputData = {}) => {
            const seq = getSeq();
            const requiredScalarData = autoGenerateNotificationItemScalarsOrEnums({ seq });
            const resolveValue = normalizeResolver(defaultDataResolver ?? {});
            const [transientFields, filteredInputData] = destructure(defaultTransientFieldValues, inputData);
            const resolverInput = { seq, ...transientFields };
            const defaultData = await traitKeys.reduce(async (queue, traitKey) => {
            ...
            const defaultAssociations = {
                Notification: isNotificationItemNotificationFactory(defaultData.Notification) ? {
                    create: await defaultData.Notification.build()
        ...
        const create = async (inputData = {}) => {
            ...
            const data = await build(inputData).then(screen);
            await handleBeforeCreate(data, transientFields);
            const createdData = await getClient().notificationItem.create({ data });
            await handleAfterCreate(createdData, transientFields);
            return createdData;
        };
        const createList = (...args) => Promise.all(normalizeList(...args).map(data => create(data)));
 ...
 */
export const defineNotificationItemFactory = ((options) => {
    return defineNotificationItemFactoryInternal(options ?? {}, {});
});
defineNotificationItemFactory.withTransientFields = defaultTransientFieldValues => options => defineNotificationItemFactoryInternal(options ?? {}, defaultTransientFieldValues);
